...
class _CreatePostScreenState extends ConsumerState<CreatePostScreen> {
  static const String _currentUserId = 'user-a-0001';
  ...
  void initState() {
    ...
    // Provider caption değişikliklerini dinle
    WidgetsBinding.instance.addPostFrameCallback((_) {
      final provider = ref.read(createPostProvider);
      provider.addListener(_syncCaption);
    });
  }
  ...
  void dispose() {
    final provider = ref.read(createPostProvider);
    provider.removeListener(_syncCaption);
  ...
  Future<void> _handleSubmit(CreatePostProvider provider) async {
    final result = await provider.submitPost(_currentUserId);
